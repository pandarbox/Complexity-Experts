import jittor
from jittor.transform import ToPILImage, Compose, RandomCrop, ToTensor
...
class Degradation(object):
    ...
    def _add_gaussian_noise(self, clean_patch, sigma):
        noise = np.random.randn(*clean_patch.shape) #生成随机噪声矩阵
        noisy_patch = np.clip(clean_patch + noise * sigma, 0, 255).astype(np.uint8) #加入到原始图像
