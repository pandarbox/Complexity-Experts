...
class _LRScheduler:
    ...
    def __init__(self, optimizer, last_epoch=-1):
        ...
        # Jittor optimizer 的 lr 存储在 optimizer.lr 而不是 param_groups['lr']
        base_lr = getattr(optimizer, 'lr', 0.001)
        self.base_lrs = [base_lr for _ in optimizer.param_groups]
        self._last_lr = self.base_lrs.copy()
        
        # 初始化 initial_lr
        for i, pg in enumerate(optimizer.param_groups):
            pg['initial_lr'] = self.base_lrs[i]
        ...
        if last_epoch == -1:
            # 初始化时设置学习率
            # Jittor optimizer 需要直接设置 optimizer.lr
            if self.base_lrs:
                optimizer.lr = self.base_lrs[0]
            for i, pg in enumerate(optimizer.param_groups):
...
class CosineAnnealingWithWarmupAndDecay(_LRScheduler):
    def __init__(self, optimizer, T_0, T_mult=2, eta_min=0.001, eta_max=0.1, 
        ...
        Args:
            optimizer: Wrapped optimizer.
            T_0 (int): Number of epochs before the first restart.
            T_mult (int): Factor to increase the period of cosine annealing after each restart.
            eta_min (float): Minimum learning rate.
            eta_max (float): Initial maximum learning rate.
    ...
    def get_lr(self):
        ...
        if self.last_epoch < self.warmup_epochs:
            ...
            if epoch_in_cycle < self.warmup_epochs:
                ...
                # 使用 math.cos 替代 torch.cos
                cos_value = math.cos((epoch_in_cycle - self.warmup_epochs) * math.pi / 
                                     (self.current_T_i - self.warmup_epochs))
                lr = self.eta_min + 0.5 * (self.current_eta_max - self.eta_min) * (1 + cos_value)
