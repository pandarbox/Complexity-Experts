...
def crop_np_image(img_np, d=32):
    return jt_to_np(crop_jt_image(np_to_jt(img_np), d))


def crop_jt_image(img, d=32):
    """
    Make dimensions divisible by d
...
def get_params(opt_over, net, net_input, downsampler=None):
    ...
    :param net: network
    :param net_input: jittor.Var that stores input `z`
    :param downsampler:
    :return:
    """
...
def get_image_grid(images_np, nrow=8):
    ...
    n_maps = len(images_np)
    x_maps = min(nrow, n_maps)
    y_maps = int(np.ceil(float(n_maps) / x_maps))
    
    height, width = images_np[0].shape[1:]
    channels = images_np[0].shape[0]
    
    padding = 2
    pad_value = 0
    
    grid_h = y_maps * height + (y_maps + 1) * padding
    grid_w = x_maps * width + (x_maps + 1) * padding
    
    grid = np.full((channels, grid_h, grid_w), pad_value, dtype=images_np[0].dtype)
    ...
    for y in range(y_maps):
        for x in range(x_maps):
            ...
            h_start = y * height + (y + 1) * padding
            h_end = h_start + height
            w_start = x * width + (x + 1) * padding
            w_end = w_start + width
            
            grid[:, h_start:h_end, w_start:w_end] = images_np[k]
            k += 1
...
def save_image_tensor(image_tensor, output_path="output/"):
    image_np = jt_to_np(image_tensor)
...
#     return crop_jt_image(data.transpose(0, 3, 1, 2).astype(np.float32) / 255.)[:35]
#
#
