...
def get_model_flops(model, input_res, print_per_layer_stat=True,
    ...
    flops_model.eval()
    start_flops_count(flops_model)
    if input_constructor:
        input = input_constructor(input_res)
        _ = flops_model(**input)
    else:
        batch = jt.zeros((1, *input_res))
        _ = flops_model(batch)

    if print_per_layer_stat:
        print_model_with_flops(flops_model)
    flops_count = flops_model.compute_average_flops_cost()
    stop_flops_count(flops_model)
...
def get_model_activation(model, input_res, input_constructor=None):
    ...
    start_activation_count(activation_model)
    if input_constructor:
        input = input_constructor(input_res)
    ...
    stop_activation_count(activation_model)
...
def print_model_with_flops(model, units='GMac', precision=3):
    ...
    apply_to_modules(model, add_extra_repr)
    print(model)
    apply_to_modules(model, del_extra_repr)
...
# Helper function to apply a function to all modules (替代 model.apply)
def apply_to_modules(module, fn):
    """Apply function fn to module and all its children recursively."""
    fn(module)
    for child in module.children():
        apply_to_modules(child, fn)
...
def start_flops_count(self):
    ...
    """
    apply_to_modules(self, add_flops_counter_hook_function)
...
def stop_flops_count(self):
    ...
    """
    apply_to_modules(self, remove_flops_counter_hook_function)
...
def reset_flops_count(self):
    ...
    """
    apply_to_modules(self, add_flops_counter_variable_or_reset)
...
def add_flops_counter_hook_function(module):
    if is_supported_instance(module):
        ...
        elif isinstance(module, (nn.ReLU, nn.PReLU, nn.LeakyReLU, nn.ReLU6)):
...
def is_supported_instance(module):
    if isinstance(module,
                  (
                          ...
                          nn.BatchNorm2d,
                          nn.Linear,
                          nn.ReLU, nn.PReLU, nn.LeakyReLU, nn.ReLU6,
                  )):
        return True
...
def start_activation_count(self):
    ...
    Call it before you run the network.
    """
    apply_to_modules(self, add_activation_counter_hook_function)
...
def stop_activation_count(self):
    ...
    Call whenever you want to pause the computation.
    """
    apply_to_modules(self, remove_activation_counter_hook_function)
...
def reset_activation_count(self):
    ...
    Resets statistics computed so far.
    """
    apply_to_modules(self, add_activation_counter_variable_or_reset)
...
def is_supported_instance_for_activation(module):
    if isinstance(module,
                  (
                          nn.Conv2d, nn.ConvTranspose2d, nn.Conv1d, nn.Linear
                  )):
        return True
